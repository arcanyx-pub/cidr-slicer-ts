...
# Bump the version (patch|minor|major), stamp the CHANGELOG, and commit. Run on
# your feature branch; the bump commit rides along in your normal PR. After
# merge, run `just publish`.
bump level:
    #!/usr/bin/env bash
    set -euo pipefail
    case "{{level}}" in
    ...
    new_version=$(npm version --no-git-tag-version "{{level}}")   # e.g. "v1.0.0"
    # Stamp the CHANGELOG's Unreleased section with the version and date.
    if grep -q '^## \[Unreleased\]' CHANGELOG.md; then
        sed -i -E "s/^## \[Unreleased\]/## [${new_version#v}] - $(date +%F)/" CHANGELOG.md
    else
